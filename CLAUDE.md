...
- **Memória de tela COMPARTILHADA entre ferramentas** (`src/memory/screenKnowledge.ts`, file-backed, sem banco): cada ferramenta Maker **lê-antes** (`loadScreen`/`summarize`) o que as outras já aprenderam e **escreve-depois** (`record`) o que descobriu — abas, campos, sinais conhecidos (`knownIssues`) e estatística de inclusões — num único `systems/<CODE>/screens/<tela>/knowledge.json` (gitignored). Assim uma ferramenta **aprende vendo o que a outra deixou registrado, sem depender de prompt humano/Claude no meio.** Já plugado em `tabExplorer` (grava abas/campos), `register` (lê-antes os sinais + grava resultado/ruído de console) e `validationTests` (grava obrigatoriedade/duplicidade provadas). É o 1º tijolo da conexão com a `KnowledgeBase` (quando o Postgres/Qdrant subir, este arquivo é a fonte a espelhar — não se reinventa, conecta-se).
...
- **⚠️ GAP / progresso (CONECTAR, não reinventar):**
  - ✅ **Fatia 1 FEITA** — memória de tela compartilhada (`screenKnowledge.ts`): `tabExplorer`/`register`/`validationTests` agora lêem-antes/escrevem-depois num `knowledge.json` por tela. Uma ferramenta já aprende com o que a outra deixou. *(falta validar ao vivo + commitar em branch própria.)*
  - ⏭️ **Fatia 2** — plugar o `crud.ts` na mesma memória (lê-antes os campos/obrigatórios que `tabs`/`validate` aprenderam; escreve-depois o resultado do C/R/U/D).
  - ⏭️ **Fatia 3** — espelhar `screenKnowledge` → `KnowledgeBase` (Prisma) quando o Postgres subir, e fazer o `orchestrator` usar os blocos robustos do `makerSession` no lugar do `ScreenMapper` genérico (que tropeça no login/iframes/abas Maker).
